...
export default function AdminPortfolioTrademarks() {
  ...
  const fetchTrademarks = async () => {
    try {
      const response = await fetch(
        `/api/admin/portfolio-trademarks?status=${filter}`,
      );
      ...
      if (response.status === 503) {
        ...
        toast.error(
          data.message || "Run database migration for trademark approvals",
        );
        setTrademarks([]);
  ...
  const handleApprove = async (trademarkId: string) => {
    try {
      const response = await fetch(
        `/api/admin/portfolio-trademarks/${trademarkId}/approve`,
        {
          method: "POST",
        },
      );
  ...
  const handleReject = async (trademarkId: string) => {
    ...
    try {
      const response = await fetch(
        `/api/admin/portfolio-trademarks/${trademarkId}/reject`,
        {
          method: "POST",
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-6">
        ...
        <p className="text-gray-600">
          Review trademarks submitted from user portfolios before they can be
          used for monitoring
        </p>
      </div>
              ...
              <div className="flex items-start justify-between gap-4 flex-col sm:flex-row">
                <div className="flex-1 min-w-0">
                  <div className="flex flex-wrap items-center gap-3 mb-2">
                    ...
                    <span
                      className={`px-3 py-1 rounded-full text-xs font-medium ${getStatusBadge(tm.approval_status)}`}
                    >
                      {tm.approval_status}
                    </span>
                  </div>
        ...
        <div className="fixed inset-0 flex items-center justify-center z-50 p-4 bg-black/20">
          <div className="bg-white rounded-xl max-w-md w-full p-6 shadow-2xl border border-gray-200">
            ...
            <p className="text-gray-600 mb-6">
              This mark will be available in the user&apos;s portfolio for
        ...
        <div className="fixed inset-0 flex items-center justify-center z-50 p-4 bg-black/20">
          <div className="bg-white rounded-xl max-w-md w-full p-6 shadow-2xl border border-gray-200">
            ...
            <p className="text-gray-600 mb-4">
              The applicant will see this reason on their portfolio.
            </p>
            <textarea
