...
export default function SearchPage() {
  ...
  const visualMatchDisplayScores = useMemo(
    () =>
      visualMatches.map((m) => visualMatchDisplayPercent(m.similarityScore)),
    [visualMatches],
  );
  ...
  const performSearch = async (searchQuery: string) => {
    ...
    try {
      const response = await fetch(
        `/api/trademarks/search?query=${encodeURIComponent(searchQuery)}&limit=50`,
      );
  ...
  const performVisualSimilarity = async (
    imageBase64: string,
    visionDataJson: string,
  ) => {
    ...
    try {
      ...
      console.log(
        `[Visual Similarity] Processing ${candidates.length} candidate images`,
      );
      ...
      // Filter to only show matches whose calibrated display percent
      // clears the visible threshold (drops noisy CLIP near-baselines).
      const filteredMatches = data.results.filter(
        (m: VisualMatch) =>
          visualMatchDisplayPercent(m.similarityScore) >=
          VISUAL_MATCH_DISPLAY_THRESHOLD,
      );
  ...
  const handleCandidateClick = (candidate: string) => {
    ...
    if (normalized) {
      router.push(
        `/search?query=${encodeURIComponent(candidate)}&mode=multi&normalized=${encodeURIComponent(normalized)}`,
      );
    }
  };
  ...
  const compareBySimilarityDesc = (a: TrademarkResult, b: TrademarkResult) => {
    // Primary: similarity descending. Treat near-equal scores (within 0.005,
    // i.e. <1% display difference) as ties so secondary sorts kick in.
    const scoreDiff = b.similarity_score - a.similarity_score;

    if (Math.abs(scoreDiff) > 0.005) return scoreDiff;

    // Tiebreaker 1: ACTIVE > PENDING > DEAD (live marks first).
    ...
    const statusCmp =
      (statusPriority[b.status_norm || ""] || 0) -
      (statusPriority[a.status_norm || ""] || 0);

    if (statusCmp !== 0) return statusCmp;

    // Tiebreaker 2: country alphabetical for deterministic ordering.
    return getCountryLabel(a.owner_country).localeCompare(
      getCountryLabel(b.owner_country),
    );
  };
          ...
          <div className="mb-8">
            <div className="flex items-center gap-3 mb-4">
              <h3 className="text-xl font-bold text-gray-900">
                Visual Matches{" "}
                {visualMatches.length > 0 && `(${visualMatches.length})`}
              </h3>
              {loadingVisual && (
                    ...
                    <div className="relative w-full aspect-square mb-3 bg-gray-100 rounded overflow-hidden">
                      <img
                        ...
                        onError={(e) => {
                          ...
                          if (parent) {
                            parent.innerHTML =
                              '<div class="flex items-center justify-center h-full text-gray-400 text-sm">Image unavailable</div>';
                          }
                        }}
                      />
                    </div>
        ...
        {!loading && results.length > 0 && (
          <div className="mb-8">
            ...
            <p className="text-gray-600 text-sm mb-4">
              Registered trademarks with similar text/names (ranked by text
              similarity)
            </p>
                          ...
                          <div className="flex items-start justify-between gap-4">
                            <div className="flex-1">
                              <div className="flex items-center gap-3 mb-2 flex-wrap">
                                ...
                                <span
                                  className={`px-3 py-1 rounded-full text-xs font-semibold border ${getOfficeBadge(result.office).className}`}
                                >
                                  {getOfficeBadge(result.office).label}
                                </span>
                                <span
                                  className={`px-3 py-1 rounded-full text-xs font-semibold border ${getRiskColor(result.risk_level)}`}
                                >
                                  {result.risk_level.replace("_", " ")}
                                </span>
                                <span
                                  className={`px-3 py-1 rounded text-xs font-medium ${getStatusColor(result.status_norm)}`}
                                >
