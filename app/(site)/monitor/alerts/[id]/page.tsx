...
export default function AlertDetailPage() {
  ...
  const handleReviewAction = async (
    reviewStatus: "REVIEWED" | "DISMISSED" | "ESCALATED",
  ) => {
    if (!alert) return;
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-200">
        <div className="max-w-5xl mx-auto px-4 py-6">
          <Link
            className="text-red-800 hover:text-red-900 font-semibold text-sm mb-2 inline-block"
            href="/monitor"
          >
            ← Back to Monitor
          </Link>
          <div className="flex items-start justify-between gap-4">
            <div className="flex-1">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                <span
                  className={`px-3 py-1 rounded-full text-sm font-semibold border ${getRiskColor(alert.risk_level)}`}
                >
                  {alert.risk_level.replace("_", " ")} RISK
                </span>
                <span
                  className={`px-3 py-1 rounded text-sm font-medium ${getStatusColor(alert.trademark_status_norm)}`}
                >
                  {alert.trademark_status_norm || "Unknown Status"}
                </span>
                <span
                  className={`px-3 py-1 rounded-full text-sm font-semibold border ${getReviewStatusColor(alert.review_status)}`}
                >
                  {alert.review_status}
                </span>
              </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white border border-gray-200 rounded-lg p-6">
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-3 gap-4">
                  ...
                  <div className="col-span-2">
                    <div className="text-sm text-gray-900">
                      <span
                        className={`px-2 py-1 rounded text-xs font-medium ${getStatusColor(alert.trademark_status_norm)}`}
                      >
                ...
                {alert.trademark_classes &&
                  alert.trademark_classes.length > 0 && (
                    <div className="grid grid-cols-3 gap-4">
                      <div className="col-span-1 text-sm font-medium text-gray-500">
                        Nice Classes
                      </div>
                      ...
                      <div>
                        ...
                        {matchDetails.sharedTokens.length > 0 ? (
                          ...
                          <div className="text-sm text-gray-500 italic">
                            No exact word matches (fuzzy match)
                          </div>
                        )}
                      </div>

                      <div className="text-xs text-gray-500 bg-gray-50 p-3 rounded">
                        <strong>Note:</strong> The similarity score is
                        calculated using trigram analysis, which compares
                        character sequences to find phonetic and visual
                        similarities even when words don't match exactly.
                      </div>
                    </div>
                  )}
                </div>
