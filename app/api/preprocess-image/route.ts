...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      `[Preprocess API] Processing ${file.name} (${file.type}, ${(file.size / 1024).toFixed(2)}KB)`,
    );
    ...
    const { normal, inverted } =
      await createPreprocessingVariants(originalBuffer);

    const duration = Date.now() - startTime;

    console.log(
      `[Preprocess API] Success: normal=${normal.length} bytes, inverted=${inverted.length} bytes, duration=${duration}ms`,
    );
