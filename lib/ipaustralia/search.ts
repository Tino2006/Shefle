import {
  getIPAUTradeMarkById,
  quickSearchIPAU,
} from "@/lib/ipaustralia/client";
import {
...
function mapNormToIPAUStatus(
  statusFilters?: Array<"ACTIVE" | "PENDING" | "DEAD">,
): IPAUStatus[] | undefined {
...
function normalizeWords(detail: IPAUTrademarkDetailResponse): string | null {
  const words =
    detail.words?.filter((w) => typeof w === "string" && w.trim().length > 0) ||
    [];
...
function normalizeDetail(
  query: string,
  detail: IPAUTrademarkDetailResponse,
  ...
  return {
    ...
    filing_date:
      detail.filingDate || detail.lodgementDate || detail.priorityDate || null,
...
export async function searchIPAustralia(
  ...
  const detailResponses = await Promise.allSettled(
    selectedIds.map((id) => getIPAUTradeMarkById(id)),
  );
