...
async function fetchAccessToken(): Promise<string> {
  ...
  const responseBodyStrategy = await requestTokenWithStrategy(
    tokenUrl,
    clientId,
    clientSecret,
    "body",
...
async function fetchIPAU<T>(path: string, init: RequestInit): Promise<T> {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `IP Australia API request failed (${response.status}) at ${path}: ${text}`,
    );
  }
...
export async function getIPAUTradeMarkById(
  id: string,
): Promise<IPAUTrademarkDetailResponse> {
  return fetchIPAU<IPAUTrademarkDetailResponse>(
